import { Gauge, Map, Route, CalendarClock } from 'lucide-react';

const cards = [
  { key: 'totalDistance', label: 'Total Distance', icon: Map, suffix: 'km' },
  { key: 'activityCount', label: 'Activity Count', icon: Gauge, suffix: '' },
  { key: 'weeklyDistance', label: 'Weekly Distance', icon: Route, suffix: 'km' },
  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {cards.map((card) => {
        ...
        return (
          <article key={card.key} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft transition hover:shadow-md">
